...
class _Call(object):
    ...
    def gt_bases(self):
        ...
        if self.called:
            ...
            try:
                return self.gt_phase_char().join(
                    str(self.site.alleles[int(X)] if X is not None else '.') for X in self.gt_alleles)
            except:
...
class _Record(object):
    ...
    @property
    def is_snp(self):
        """ Return whether the variant is an SNP """
        if len(self.REF) > 1:
            return False
        for alt in self.ALT:
    ...
    @property
    def is_indel(self):
        """ Return whether the variant is an INDEL """
        is_sv = self.is_sv
...
class _AltRecord(object):
    ...
    def __init__(self, type, **kwargs):
        ...
        #: String to describe the type of variant, by default "SNV" or "MNV",
        # but can be extended to any of the types described in the ALT lines of the header (e.g. "DUP", "DEL", "INS"...)
        self.type = type
...
class _Breakend(_AltRecord):
    ...
    def __init__(self, chr, pos, orientation, remoteOrientation, connectingSequence, withinMainAssembly, **kwargs):
        ...
        if pos is not None:
            ...
            self.pos = None
        #: The orientation of breakend's mate. If the sequence 3' of the breakend's mate is connected,
        # True, else if the sequence 5' of the breakend's mate is connected, False.
        self.remoteOrientation = remoteOrientation
        #: If the breakend mate is within the assembly, True,
        # else False if the breakend mate is on a contig in an ancillary assembly file.
        self.withinMainAssembly = withinMainAssembly
        #: The orientation of breakend. If the sequence 3' of the breakend is connected,
        # True, else if the sequence 5' of the breakend is connected, False.
